...
pub struct TableSchema {
    ...
    ///
    /// This is pre-computed during construction, so it can be returned as a
    /// cheap reference.
...
impl TableSchema {
    ...
    pub fn with_virtual_columns(mut self, virtual_columns: Vec<FieldRef>) -> Self {
        ...
        if self.virtual_columns.is_empty() {
            ...
            existing.extend(virtual_columns);
        }
        self.table_schema = build_table_schema(
            &self.file_schema,
            self.table_partition_cols.as_ref(),
    ...
    /// partition paths.
    pub fn virtual_columns(&self) -> &Vec<FieldRef> {
...
mod tests {
    ...
    #[test]
    fn test_with_virtual_columns_layout() {
        let file_schema = Arc::new(Schema::new(vec![
            Field::new("user_id", DataType::Int64, false),
            Field::new("amount", DataType::Float64, false),
        ]));
        ...
        // Apply virtual columns and partition columns in either order; the
        // resulting table schema should always be [file, partition, virtual].
        let built_virtual_first = TableSchema::from_file_schema(Arc::clone(&file_schema))
