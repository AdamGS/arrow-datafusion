...
fn can_swap_null_aware_hash_join(hash_join: &HashJoinExec) -> bool {
...
fn null_aware_hash_join_mode(hash_join: &HashJoinExec) -> PartitionMode {
    ...
    match (left_can_collect, right_can_collect) {
        (true, true) => {
            ...
            if hash_join.join_type().supports_swap()
                && can_swap_null_aware_hash_join(hash_join)
...
pub(crate) fn partitioned_hash_join(
    ...
    let left = hash_join.left();
    let right = hash_join.right();
    let partition_mode = null_aware_hash_join_mode(hash_join);
    // Don't swap null-aware anti joins as they have specific side requirements
    if hash_join.join_type().supports_swap()
        && can_swap_null_aware_hash_join(hash_join)
        && should_swap_join_order(&**left, &**right, config)?
    ...
    let transformed = if let Some(hash_join) = plan.downcast_ref::<HashJoinExec>() {
        match hash_join.partition_mode() {
            ...
            PartitionMode::Partitioned => {
                ...
                } else if *hash_join.partition_mode() != partition_mode {
                    Some(Arc::new(
                        hash_join
                            .builder()
                            .with_partition_mode(partition_mode)
